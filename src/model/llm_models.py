...
class LLMManager:
    ...
    def __init__(self):
        ...
        # 2. Mappa environment DEFAULT_MODEL -> vilken model_key i self.models som ska väljas
        #    Om du anger "lmstudio" i .env => "lm-studio-local"
        #    Om du anger "anthropic" i .env => "claude-3-opus" (eller annan default)
        #    etc. Faller tillbaka till "lm-studio-local" om inget matchar.
        provider_map = {
        ...
        # 3. Sätt current_model till rätt nyckel
        self.current_model = provider_map.get(env_provider, "lm-studio-local")

        # 4. Ladda API-nycklar, initiera klienter
        self._load_api_keys()
        self._initialize_clients()

    def _load_default_models(self) -> Dict[str, ModelConfig]:
        """
        Ladda uppsättning av standardmodeller i en dictionary,
        med nyckeln som representerar modell-namnet.
        """
        return {
